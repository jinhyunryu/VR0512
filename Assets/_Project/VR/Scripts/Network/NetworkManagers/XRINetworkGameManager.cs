...
# endif
            ...
            if (CurrentSessionType == SessionType.DistributedAuthority)
            {
                ...
                if (!signedIn)
                {
                    ...
                    ShowHudText($"Failed to Authenticate.");
                    return;
                }
            }
        ...
        void SessionOwnerPromoted(ulong sessionOwnerId)
        {
            ...
            {
                ShowHudText($"<b>Status:</b> {player.playerName} now the Host.");
            }
        }
        ...
        public virtual void OnLocalClientStarted(ulong localPlayerId)
        {
            ...
            ShowHudText($"<b>Status:</b> Connected");
        ...
        protected virtual void LocalClientStopped(bool id)
        {
            ...
            ShowHudText($"<b>Status:</b> Disconnected");
        ...
        static void ShowHudText(string text)
        {
            if (PlayerHudNotification.Instance != null)
                PlayerHudNotification.Instance.ShowText(text);
            else
                Utils.Log($"{k_DebugPrepend}{text}", 0);
        }
