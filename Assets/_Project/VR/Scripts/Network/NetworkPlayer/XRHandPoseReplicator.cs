    ...
    public class XRHandPoseReplicator : NetworkBehaviour
    {
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if (IsOwner)
            {
                ...
                m_XRModalityManager = ArcadeLocalRigReference.FindInputModalityManager();

                if (m_XRModalityManager != null)
                {
                    SetupLocalHands();
                }
                else if (ArcadeLocalRigReference.TryFindControllerTransforms(out Transform left, out Transform right))
                {
                    m_LeftControllerTransformReference = left;
                    m_RightControllerTransformReference = right;
                    m_LeftHandTransformReference = left;
                    m_RightHandTransformReference = right;
        ...
        {
            XRInputModalityManager modalityMangager = m_XRModalityManager != null
                ? m_XRModalityManager
                : ArcadeLocalRigReference.FindInputModalityManager();
